...
export default function GoogleSignInButton({ onCredential, disabled }) {
  ...
  useEffect(() => {
    ...
    if (!clientId) {
      console.warn('VITE_GOOGLE_CLIENT_ID is missing. Google Sign-In is disabled.');
      return;
    }
  ...
  if (!import.meta.env.VITE_GOOGLE_CLIENT_ID) {
    return (
      <button 
        disabled
        className="w-full py-2.5 px-4 rounded border border-[#d6c1c9] bg-gray-50 text-gray-400 text-sm font-medium flex items-center justify-center gap-2 cursor-not-allowed"
      >
