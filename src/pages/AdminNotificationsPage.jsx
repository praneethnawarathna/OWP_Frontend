...
const getTypeIcon = (type) => {
  switch (type) {
    ...
    case 'ListingCreated':
      return <ClipboardList size={15} className="text-[#8E406F]" />;
    ...
    case 'SecurityChange':
      return <ShieldAlert size={15} className="text-purple-600" />;
    default:
      return <Bell size={15} className="text-[#8E406F]" />;
  }
};

export default function AdminNotificationsPage({ onNavigate }) {
  ...
  const filteredNotifications = useMemo(() => {
    ...
    // Pin AiApprovalRequired items to the top for admin attention, then chronological
    return [...list].sort((a, b) => {
      const aIsAi = a.type === 'AiApprovalRequired' || a.Type === 'AiApprovalRequired';
      const bIsAi = b.type === 'AiApprovalRequired' || b.Type === 'AiApprovalRequired';
  ...
  return (
    <PageShell
      ...
      description="Monitor critical platform updates, pending vendor actions, listing requests, and AI governance approvals."
      icon={Bell}
    >
      <div className="space-y-4">
        ...
        {error ? (
          ...
          <div className="rounded-2xl border border-[#F1E5EC] bg-white p-12 text-center shadow-sm">
            ...
            <p className="mt-3 text-sm font-medium text-[#737373]">Loading notifications...</p>
          </div>
          ...
          <div className="rounded-2xl border border-dashed border-[#E8DDE4] bg-white p-12 text-center shadow-sm">
            ...
            <p className="mt-1 text-xs text-[#737373] max-w-xs mx-auto">
              {filter === 'unread'
                ? 'All administrator alerts and notices have been reviewed.'
                : 'Platform events, vendor submissions, and AI approval requests will appear here.'}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {filteredNotifications.map((item) => {
              ...
              const isAiApproval =
                item.type === 'AiApprovalRequired' || item.Type === 'AiApprovalRequired';
              ...
              return (
                <article
                  ...
                  className={`group relative flex items-start justify-between gap-4 rounded-2xl border p-5 shadow-sm transition cursor-pointer ${
                    isAiApproval
                      ? isUnread
                        ? 'border-amber-300 border-l-4 border-l-amber-500 bg-amber-50/40 hover:bg-amber-50/70 hover:shadow-md'
                        : 'border-amber-200 border-l-4 border-l-amber-400 bg-amber-50/20 hover:bg-amber-50/40'
                      : isUnread
                  ...
                  <div className="flex items-start gap-3.5 flex-1 min-w-0">
                    ...
                    <div className="space-y-1.5 flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {/* Distinct AI Approval Badge or Regular Type Badge */}
                        {isAiApproval ? (
                          <span className="inline-flex items-center gap-1 rounded-lg bg-amber-100 border border-amber-300 px-2 py-0.5 text-[10px] font-bold text-amber-800 shadow-xs">
